...
class TestSelectTopKeywords:
    ...
    def test_raises_error_on_invalid_limit() -> None:
        ...
        with pytest.raises(ValueError):
            _select_top_keywords(
                [{"term": "alpha", "score": 1.0}], limit=0, include_ties=False
            )


@pytest.mark.asyncio
async def test_aggregator_node_applies_llm_verification(
    ...
    monkeypatch.setattr(
        aggregator_module, "cluster_keywords_by_embedding", fake_cluster
    )
    monkeypatch.setattr(
        aggregator_module, "aggregate_clustered_keywords", fake_aggregate
    )
...
@pytest.mark.asyncio
async def test_aggregator_node_skips_llm_when_disabled(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """
    ...
    class FailingGemini:
        def __init__(self, *_args: Any, **_kwargs: Any) -> None:
            pytest.fail(
                "GeminiClient should not be instantiated when verification is disabled."
            )
