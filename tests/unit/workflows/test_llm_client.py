...
LLM_CLIENT_MODULE_PATH = (
    Path(__file__).resolve().parents[3] / "src" / "workflows" / "llm_client.py"
)
LLM_CLIENT_SPEC = importlib.util.spec_from_file_location(
    "llm_client_module", LLM_CLIENT_MODULE_PATH
)
...
class TestGeminiClientAPI:
    ...
    @patch("src.workflows.llm_client.LANGCHAIN_AVAILABLE", True)
    @patch("src.workflows.llm_client.ChatGoogleGenerativeAI")
    async def test_generate_content_async_with_json_format(
        ...
        result = await client.generate_content_async(
            "test prompt", response_format="json"
        )
    ...
    @patch("src.workflows.llm_client.LANGCHAIN_AVAILABLE", True)
    @patch("src.workflows.llm_client.ChatGoogleGenerativeAI")
    async def test_generate_content_async_raises_on_empty_response(
        mock_chat_class: MagicMock,
    ) -> None:
        """
