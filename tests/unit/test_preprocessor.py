...
class TestPreprocessor:
    ...
    async def test_preprocess_collected_items_split_long_messages(self, caplog):
        ...
        original_total_tokens = len(
            preprocessor.token_encoder.encode(normalized_long_text)
        )
        effective_overlap = preprocessor._resolve_overlap_tokens(
        ...
        for index, segment in enumerate(result):
            ...
            assert (
                segment.metadata["original_message_id"] == item.metadata["message_id"]
            )
            ...
            if index == 0:
                ...
                assert range_info["start"] == max(
                    previous_range_end - effective_overlap, 0
                )
        ...
        stats_log = next(
            (message for message in caplog.messages if "분할세그먼트" in message), ""
        )
    ...
    def test_split_text_to_segments_token_based(self):
        ...
        for index, segment in enumerate(segments, start=1):
            ...
            assert (
                0 < token_count <= config.max_tokens_per_segment
    ...
    async def test_chunk_messages_source_grouping(self, preprocessor):
        ...
        for chunk in chunks:
            sources = {record.get("source") for record in chunk}
    ...
    async def test_chunk_messages_time_sorting(self, preprocessor):
        ...
        for chunk in chunks:
            timestamps = [record.get("timestamp") for record in chunk]
            # 시간순으로 정렬되어 있는지 확인
