...
@pytest.mark.asyncio
async def test_collect_rss_data_empty_sources(
    execution_logger, app_config_with_telegram_only
):
    """RSS 소스가 없을 때 정상 동작 테스트"""
    min_timestamp = datetime.now(timezone.utc)

    articles = await collect_rss_data(
        app_config_with_telegram_only, min_timestamp, execution_logger
    )
...
@pytest.mark.asyncio
async def test_collect_telegram_data_empty_sources(
    execution_logger, app_config_with_rss_only
):
    """Telegram 소스가 없을 때 정상 동작 테스트"""
    min_timestamp = datetime.now(timezone.utc)

    messages = await collect_telegram_data(
        app_config_with_rss_only, min_timestamp, execution_logger
    )
...
@pytest.mark.asyncio
async def test_collect_rss_data_with_sources(
    execution_logger, app_config_with_both_sources
):
    ...
    with patch("main.RSSCollector") as mock_collector_class:
        ...
        articles = await collect_rss_data(
            app_config_with_both_sources, min_timestamp, execution_logger
        )
...
@pytest.mark.asyncio
async def test_collect_telegram_data_with_sources(
    execution_logger, app_config_with_both_sources
):
    ...
    with patch("main.StorageManager") as mock_storage_class:
        ...
        with patch("main.setup_logging"):
            ...
            with patch.object(ExecutionLogger, "log_error", mock_log_error):
                ...
                # 에러가 로그에 기록되었는지 확인
                no_source_errors = [
                    e for e in captured_errors if e["type"] == "NoSourceError"
                ]
                assert len(no_source_errors) == 1
                assert (
                    "수집할 소스가 설정되지 않았습니다"
                    in no_source_errors[0]["message"]
                )
