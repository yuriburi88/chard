...
async def test_economic_calendar():
    ...
    try:
        ...
        print(
            f"   버전: {investpy.__version__ if hasattr(investpy, '__version__') else 'Unknown'}"
        )
    except ImportError:
    ...
    try:
        ...
        if calendar_df.empty:
            ...
            for idx, row in calendar_df.head(3).iterrows():
                print(f"\n이벤트 #{idx + 1}:")
                print(
                    f"  - 날짜/시간: {row.get('date', 'N/A')} {row.get('time', 'N/A')}"
                )
    ...
    try:
        ...
        records = await collect_economic_calendar(
            countries=["united states"], importance="high", days_back=7
        )
