...
class FakeBot:
    ...
    async def get_chat(self, channel_id: str) -> SimpleNamespace:
        return SimpleNamespace(title="Crypto News", username="cryptonews", id=-100123456)
...
async def test_bot_api_collector_collect(monkeypatch) -> None:
    ...
    monkeypatch.setattr("src.collectors.telegram_collector.Bot", lambda token, request=None: fake_bot)

    async with collector:
        messages = await collector.collect(config, min_timestamp=message_time - timedelta(minutes=5))
...
async def test_bot_api_collector_time_filtering(monkeypatch, sample_telegram_config):
    ...
    fake_bot = FakeBot(token=sample_telegram_config.bot_token, request=collector.request_timeout)
    fake_bot.set_updates(fake_updates)
    
    monkeypatch.setattr("src.collectors.telegram_collector.Bot", lambda token, request=None: fake_bot)
    
    async with collector:
        messages = await collector.collect(sample_telegram_config, min_timestamp=min_timestamp)
...
@pytest.mark.asyncio
async def test_bot_api_collector_max_messages_limit(monkeypatch, sample_telegram_config):
    """
...
async def test_telegram_collector_factory_bot_api(sample_telegram_config):
    ...
    """
    collector = TelegramCollectorFactory.create(sample_telegram_config, request_timeout=5.0)
...
async def test_telegram_collector_factory_mtproto(sample_mtproto_config):
    ...
    """
    collector = TelegramCollectorFactory.create(sample_mtproto_config, request_timeout=5.0)
...
async def test_mtproto_collector_mock(sample_mtproto_config):
    ...
    mock_client_instance.get_entity = AsyncMock(return_value=SimpleNamespace(
        id=-100123456,
    ...
    with patch.dict('sys.modules', {
        ...
        'telethon.errors': mock_telethon_errors
    }):
