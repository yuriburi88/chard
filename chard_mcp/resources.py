...
async def read_latest_report():
    ...
    if not output_dir.exists():
        return [TextContent(
            type="text",
            text="리포트를 찾을 수 없습니다. 먼저 분석을 실행하세요."
        )]
    ...
    if not report_files:
        return [TextContent(
            type="text",
            text=f"{latest_dir.name}의 리포트 파일을 찾을 수 없습니다."
        )]
...
async def read_latest_data():
    ...
    if not data_files:
        return [TextContent(
            type="text",
            text=f"{date_str}의 수집 데이터 파일을 찾을 수 없습니다."
        )]
...
async def read_latest_analysis():
    ...
    if not analysis_files:
        return [TextContent(
            type="text",
            text=f"{date_str}의 분석 결과를 찾을 수 없습니다."
        )]
