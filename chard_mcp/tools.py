    ...
    if feed_name:
        ...
        return [TextContent(
            ...
            text=f"RSS 피드 '{feed_name}'에서 최근 {hours_back}시간 동안의 기사 수집을 시작합니다.\n\n" +
                 f"이 작업은 실제 main.py를 실행하여 수집합니다.\n" +
                 f"현재는 데모 모드로, 실제 구현은 main.py와 통합이 필요합니다."
        )]
    else:
        # 모든 피드 수집
...
async def handle_get_latest_keywords(args: dict):
    ...
    if not analysis_files:
        return [TextContent(
            type="text",
            text=f"{date_str}의 분석 결과 파일을 찾을 수 없습니다."
        )]
    ...
    if not keywords:
        return [TextContent(
            type="text",
            text=f"{date_str}의 키워드 데이터가 비어있습니다."
        )]
