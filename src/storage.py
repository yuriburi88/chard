...
try:
    ...
    logging.warning("aiofiles가 설치되지 않았습니다. 비동기 파일 I/O 기능이 비활성화됩니다.")
...
class StorageManager:
    ...
    async def create_output_directory(self, timestamp: Optional[datetime] = None) -> Path:
        """
        출력 디렉토리 구조를 생성합니다.
        ...
        logger.info(
            f"[StorageManager] 출력 디렉토리 생성: {output_dir}"
        )
    ...
    async def load_json(self, file_path: Path | str) -> Dict[str, Any] | List[Dict[str, Any]]:
        """
        JSON 파일을 비동기로 읽어옵니다.
    ...
    async def save_raw_data(
        ...
        logger.info(
            f"[StorageManager] 원본 데이터 저장 완료: {file_path}"
        )
...
class ExecutionLogger:
    ...
    def start(self) -> None:
        ...
        logger.info(
            f"[ExecutionLogger] 실행 시작: {self.start_time.isoformat()}"
        )
    ...
    def set_statistics(self, stats: Dict[str, Any]) -> None:
        ...
        logger.info(
            f"[ExecutionLogger] 통계 정보 설정: {stats}"
        )
    ...
    def add_statistic(self, key: str, value: Any) -> None:
        ...
        logger.debug(
            f"[ExecutionLogger] 통계 추가: {key} = {value}"
        )
    ...
    async def save_log(
        ...
        filename: str = "execution_log.json"
    ) -> Path:
        """
        ...
        logger.info(
            f"[ExecutionLogger] 실행 로그 저장 완료: {file_path}"
        )
