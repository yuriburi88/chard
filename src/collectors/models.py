...
MetadataValue = Union[
    str, int, float, bool, list[str], list[int], SegmentRangeTokens, None
]
...
class TelegramMessage:
    ...
    # 선택적 필드
    author: str | None = None  # 작성자 (사용자명)
    reply_to_message_id: int | None = None  # 답장인 경우 원본 메시지 ID
...
class Article:
    ...
    description: str | None = None  # 기사 요약
    author: str | None = None  # 작성자 (RSS의 경우)
    tags: list[str] | None = None  # 태그 리스트

    # 텔레그램 전용 필드 (source_type="telegram"일 때 사용)
    telegram_message_ids: list[int] | None = None  # 포함된 메시지 ID 리스트
    telegram_message_count: int | None = None  # 포함된 메시지 개수
    ...
    def from_dict(cls, data: dict) -> "Article":
        ...
        if isinstance(published_at, str):
            ...
            raise ValueError(
                "published_at은 datetime 또는 ISO 형식 문자열이어야 합니다."
            )
...
class CollectedItem:
    ...
    )  # 부가 메타데이터 (예: message_id, segment_range_tokens 등)

    def __post_init__(self) -> None:
        """데이터 검증 및 정규화"""
        if self.source_type not in ("rss", "telegram", "economic_calendar"):
            raise ValueError(
                "source_type은 'rss', 'telegram', 또는 'economic_calendar'이어야 합니다."
            )
