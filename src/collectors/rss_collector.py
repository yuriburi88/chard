...
class RSSCollector:
    ...
    async def collect(
        ...
        logger.info(f"[RSS 수집 설정] max_articles={config.max_articles}, hours_back={config.hours_back}, timezone=UTC+{config.timezone}")
        ...
        try:
            ...
            if feed.bozo:
                logger.warning(f"[RSS 피드 파싱 경고] {config.name}: {feed.bozo_exception}")
            
            if not feed.entries:
                logger.warning(f"[RSS 피드 비어있음] {config.name}: 기사가 없습니다.")
                return []
            
            logger.info(f"[RSS 피드 파싱 완료] {config.name}: {len(feed.entries)}개 엔트리 발견")
            ...
            if min_published is None:
                # hours_back을 기준으로 계산
                min_published = datetime.now(timezone.utc) - timedelta(hours=config.hours_back)
                logger.info(f"[RSS 시간 필터] 최소 발행 시각: {min_published.isoformat()} (최근 {config.hours_back}시간)")
            
            # 엔트리 처리
            for idx, entry in enumerate(feed.entries):
                # 최대 기사 수 확인
                if len(articles) >= config.max_articles:
                    logger.info(f"[RSS 수집 중단] {config.name}: 최대 기사 수({config.max_articles}) 도달")
                    break
                ...
                try:
                    ...
                    if unique_key in seen_keys:
                        logger.debug(f"[RSS 중복 제거] {config.name}: {article.title[:50]}...")
                        continue
                    seen_keys.add(unique_key)
                    
                    articles.append(article)
                    logger.info(f"[RSS 기사 수집] {config.name} [{len(articles)}/{config.max_articles}]: {article.title}")
                    logger.debug(f"[RSS 기사 상세] URL: {article.url}, 발행: {article.published_at.isoformat()}, 본문 길이: {len(article.content)}자")
                    
                except Exception as e:
                    logger.error(f"[RSS 엔트리 파싱 실패] {config.name} 엔트리 {idx+1}: {e}", exc_info=True)
                    continue
            
            logger.info(f"[RSS 수집 완료] {config.name}: 총 {len(articles)}개 기사 수집")
    ...
    async def _fetch_feed(self, url: str) -> feedparser.FeedParserDict:
        ...
        try:
            ...
            logger.debug(f"[RSS HTTP 응답] 상태 코드: {response.status_code}, 본문 길이: {len(response.content)} bytes")
            
            # feedparser는 동기 라이브러리이므로 asyncio.to_thread로 래핑
            feed = await asyncio.to_thread(feedparser.parse, response.content)
            
            logger.debug(f"[RSS 피드 파싱] 엔트리 수: {len(feed.entries) if feed.entries else 0}")
    ...
    def _parse_entry(
        ...
        config: RSSSourceConfig,
        min_published: Optional[datetime]
    ) -> Optional[Article]:
        """
        ...
        if not title:
            logger.warning(f"[RSS 엔트리 파싱] 제목이 비어있어 스킵합니다.")
            return None
        ...
        if not link:
            logger.warning(f"[RSS 엔트리 파싱] 링크가 비어있어 스킵합니다. 제목: {title[:50]}...")
            return None
        
        # 발행 시각 파싱
        published_at = self._parse_entry_date(entry)
        if published_at is None:
            logger.warning(f"[RSS 엔트리 파싱] 발행 시각을 파싱할 수 없어 스킵합니다. 제목: {title[:50]}...")
            return None
    ...
    def _get_entry_value(self, entry: feedparser.FeedParserDict, key: str, default: str = '') -> str:
        """
        엔트리에서 값을 안전하게 추출합니다.
            ...
            if hasattr(entry, field):
                ...
                if date_str:
                    try:
                        ...
                        logger.debug(f"[RSS 날짜 파싱 실패] 필드: {field}, 값: {date_str}, 오류: {e}")
                        continue
