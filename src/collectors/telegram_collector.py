...
class BaseTelegramCollector(BaseCollector, ABC):
    """
    텔레그램 수집기 추상 기본 클래스
    ...
    @abstractmethod
    async def collect(
        self, config: TelegramSourceConfig, min_timestamp: datetime | None = None
    ...
    def _clean_messages(self, messages: list[TelegramMessage]) -> list[TelegramMessage]:
        ...
        for msg in messages:
            ...
            if len(msg.text.strip()) <= 1:
                logger.debug(
                    f"[메시지 청소] 너무 짧은 메시지 제외: ID={msg.message_id}, 길이={len(msg.text)}"
                )
                continue
            ...
            if any(stop_word in text_lower for stop_word in stop_words):
                logger.debug(
                    f"[메시지 청소] 불용어 포함 메시지 제외: ID={msg.message_id}, 텍스트={msg.text[:50]}..."
                )
                continue
    ...
    def _calculate_min_timestamp(
        self, config: TelegramSourceConfig, min_timestamp: datetime | None
    ) -> datetime:
        """
        최소 타임스탬프를 계산합니다.
        ...
        if min_timestamp is None:
            ...
            logger.info(
                f"[텔레그램 시간 필터] 최소 타임스탬프: {min_timestamp.isoformat()} (최근 {hours_back}시간)"
            )
...
class BotAPICollector(BaseTelegramCollector):
    ...
    async def collect(
        ...
        if not config.bot_token:
            raise ValueError(
                f"Bot API를 사용하려면 bot_token이 필요합니다. (소스: {config.name})"
            )

        logger.info("[Bot API] 봇 초기화 중...")
        ...
        try:
            ...
            logger.info(
                f"[Bot API] 봇 정보: @{bot_info.username} ({bot_info.first_name})"
            )
            ...
            try:
                ...
                logger.warning(
                    f"[Bot API] 채널 정보 조회 실패: {e}, 채널명을 설정값으로 사용"
                )
            ...
            try:
                ...
                for update in updates:
                    if update.channel_post:
                        ...
                        # 채널 ID 확인
                        if msg.chat.username == config.channel_id.replace(
                            "@", ""
                        ) or str(msg.chat.id) == config.channel_id.replace("-", ""):
                            # 메시지 타임스탬프 확인
            ...
            logger.info(
                f"[텔레그램 메시지 청소] {len(messages)}개 → {len(cleaned_messages)}개"
            )
...
class MTProtoCollector(BaseTelegramCollector):
    ...
    async def collect(
        ...
        if not config.api_id or not config.api_hash:
            raise ValueError(
                f"MTProto를 사용하려면 api_id와 api_hash가 필요합니다. (소스: {config.name})"
            )
        ...
        try:
            ...
            raise ImportError(
                "MTProto를 사용하려면 telethon 패키지가 필요합니다. 'pip install telethon'을 실행하세요."
            )

        logger.info("[MTProto] 클라이언트 초기화 중...")
        ...
        if config.session_file:
            ...
            session_name = (
                f"telegram_{config.channel_id.replace('@', '').replace('-', '')}"
            )
        ...
        try:
            ...
            try:
                ...
                # 세션이 만료되었거나 무효화된 경우
                logger.error(
                    f"[MTProto] 세션이 만료되었거나 무효화되었습니다. (소스: {config.name})"
                )
                raise ValueError(
                ...
                # 기타 연결 오류
                logger.error(
                    f"[MTProto] 클라이언트 연결 중 오류 발생: {e} (소스: {config.name})"
                )
                raise ValueError(
            ...
            if not is_authorized:
                logger.warning(
                    f"[MTProto] 세션이 만료되었거나 인증되지 않았습니다. (소스: {config.name})"
                )
                logger.warning(
                    "[MTProto] 세션 재인증이 필요합니다. 'tele_setup.py'를 실행하여 세션을 갱신하세요."
                )
            ...
            logger.info("[MTProto] 클라이언트 연결 완료")
            ...
            try:
                ...
                # 채널 조회 중 세션 만료 감지
                logger.error(
                    f"[MTProto] 채널 조회 중 세션이 만료되었습니다. (소스: {config.name})"
                )
                raise ValueError(
                    f"MTProto 세션이 만료되었습니다. (소스: {config.name})\n"
                    f"다음 명령을 실행하여 세션을 재생성하세요:\n"
                ...
                # API 제한으로 인한 대기 필요
                logger.warning(
                    f"[MTProto] API 제한으로 인해 {e.seconds}초 대기 필요합니다. (소스: {config.name})"
                )
                raise ValueError(
                ...
                logger.warning(
                    f"[MTProto] 채널 정보 조회 실패: {e}, 채널명을 설정값으로 사용"
                )
                channel_name = config.name
            ...
            try:
                ...
                logger.error(
                    f"[MTProto] 메시지 수집 시작 전 세션이 만료되었습니다. (소스: {config.name})"
                )
                raise ValueError(
                    f"MTProto 세션이 만료되었습니다. (소스: {config.name})\n"
                    f"다음 명령을 실행하여 세션을 재생성하세요:\n"
            ...
            try:
                async for msg in message_iter:
                    ...
                    if msg.date:
                        ...
                        if min_timestamp and msg_timestamp < min_timestamp:
                            logger.debug(
                                "[MTProto] 시간 필터링: 메시지가 최소 타임스탬프보다 이전입니다. 수집 중단."
                            )
                            break
                    ...
                    telegram_msg = TelegramMessage(
                        ...
                        channel_name=channel_name,
                        timestamp=(
                            msg_timestamp if msg.date else datetime.now(timezone.utc)
                        ),
                        author=(
                    ...
                    messages.append(telegram_msg)
                    logger.debug(
                        f"[MTProto] 메시지 수집: ID={msg.id}, 텍스트 길이={len(text)}"
                    )
                ...
                # 메시지 수집 중 세션 만료 감지
                logger.error(
                    f"[MTProto] 메시지 수집 중 세션이 만료되었습니다. (소스: {config.name}, 수집된 메시지: {len(messages)}개)"
                )
                raise ValueError(
                    f"MTProto 세션이 만료되었습니다. (소스: {config.name})\n"
                    f"다음 명령을 실행하여 세션을 재생성하세요:\n"
                ...
                # 메시지 수집 중 API 제한
                logger.warning(
                    f"[MTProto] 메시지 수집 중 API 제한으로 인해 {e.seconds}초 대기 필요합니다. (소스: {config.name}, 수집된 메시지: {len(messages)}개)"
                )
                raise ValueError(
                    f"Telegram API 제한으로 인해 {e.seconds}초 대기 후 다시 시도해야 합니다. (소스: {config.name})"
                )
...
class TelegramCollectorFactory:
    ...
    @staticmethod
    def create(
        config: TelegramSourceConfig, request_timeout: float = 30.0
...
class TelegramCollector:
    ...
    def __init__(self, request_timeout: float = 30.0):
        ...
        """
        self.request_timeout = request_timeout
        self._collector: BaseTelegramCollector | None = None
    ...
    async def collect(
        ...
        """
        logger.info(
            f"[텔레그램 수집 시작] 소스: {config.name}, 채널: {config.channel_id}"
        )
        logger.info(
            f"[텔레그램 수집 설정] auth_method={config.auth_method}, max_messages={config.max_messages}, timezone=UTC+{config.timezone}"
        )
        ...
        try:
            ...
            logger.info(
                f"[텔레그램 수집 완료] {config.name}: 총 {len(messages)}개 메시지 수집"
            )
