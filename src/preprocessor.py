...
try:
    ...
    logging.warning(
        "langdetect가 설치되지 않았습니다. 언어 감지 기능이 비활성화됩니다."
    )

import tiktoken

from src.collectors.models import Article, CollectedItem, MetadataValue
...
class Preprocessor:
    ...
    def __init__(self, config: PreprocessingConfig | None = None):
        ...
        try:
            ...
            logger.info(
                f"[Preprocessor] 토큰 인코더 초기화 완료: {self.config.token_encoding}"
            )
        ...
        if self.config.language_detection_enabled and not LANGDETECT_AVAILABLE:
            logger.warning(
                "[Preprocessor] langdetect가 설치되지 않아 언어 감지가 비활성화됩니다."
            )
    ...
    async def preprocess_articles(self, articles: list[Article]) -> list[Article]:
        """
        Article 리스트를 전처리합니다.
        ...
        for article in articles:
            try:
                ...
                if not article.content or not article.content.strip():
                    logger.debug(
                        f"[Preprocessor] 빈 본문 제외: source={article.source}, title={article.title[:50]}"
                    )
                ...
                if (
                    ...
                    or len(normalized_content.strip()) < self.config.min_content_length
                ):
                    logger.debug(
                        f"[Preprocessor] 너무 짧은 본문 제외: source={article.source}, 길이={len(normalized_content)}"
                    )
                    stats["length_filtered"] += 1
                    continue
                ...
                if len(normalized_content) > self.config.max_content_length:
                    logger.debug(
                        f"[Preprocessor] 너무 긴 본문 제외: source={article.source}, 길이={len(normalized_content)}"
                    )
                    stats["length_filtered"] += 1
                    continue
                ...
                if self.config.language_detection_enabled:
                    ...
                    if (
                        detected_lang
                        and detected_lang not in self.config.allowed_languages
                    ):
                        logger.debug(
                            f"[Preprocessor] 허용되지 않은 언어 제외: source={article.source}, 언어={detected_lang}"
                        )
                ...
                if self.config.spam_filter_enabled:
                    if self._is_spam(normalized_content) or self._is_spam(
                        normalized_title
                    ):
                        logger.debug(
                            f"[Preprocessor] 스팸으로 판단된 항목 제외: source={article.source}, title={article.title[:50]}"
                        )
                ...
                if self.config.token_limit_enabled and self.token_encoder:
                    ...
                    if token_count > self.config.max_tokens_per_item:
                        logger.debug(
                            f"[Preprocessor] 토큰 제한 초과 제외: source={article.source}, 토큰={token_count}"
                        )
                ...
                processed_article = Article(
                    ...
                    description=(
                        self._normalize_text(article.description)
                ...
                logger.error(
                    f"[Preprocessor] Article 전처리 중 오류 발생: source={article.source}, error={e}"
                )
                continue
        ...
        for item in items:
            try:
                ...
                if not item.text or not item.text.strip():
                    logger.debug(
                        f"[Preprocessor] 빈 텍스트 제외: source={item.source_name}"
                    )
                    stats["length_filtered"] += 1
                    continue
                ...
                if (
                    not normalized_text
                    or len(normalized_text.strip()) < self.config.min_content_length
                ):
                    logger.debug(
                        f"[Preprocessor] 너무 짧은 텍스트 제외: source={item.source_name}, 길이={len(normalized_text)}"
                    )
                    stats["length_filtered"] += 1
                    continue
                ...
                if len(normalized_text) > self.config.max_content_length:
                    logger.debug(
                        f"[Preprocessor] 너무 긴 텍스트 제외: source={item.source_name}, 길이={len(normalized_text)}"
                    )
                    stats["length_filtered"] += 1
                    continue
                ...
                if self.config.language_detection_enabled:
                    ...
                    if (
                        ...
                        logger.debug(
                            f"[Preprocessor] 허용되지 않은 언어 제외: source={item.source_name}, 언어={detected_lang}"
                        )
                        stats["language_filtered"] += 1
                        continue
                ...
                if self.config.spam_filter_enabled:
                    if self._is_spam(normalized_text):
                        logger.debug(
                            f"[Preprocessor] 스팸으로 판단된 항목 제외: source={item.source_name}"
                        )
                        stats["spam_filtered"] += 1
                        continue
                ...
                logger.error(
                    f"[Preprocessor] CollectedItem 전처리 중 오류 발생: source={item.source_name}, error={e}"
                )
                continue
    ...
    def _split_collected_item(
        self, original_item: CollectedItem, normalized_text: str, token_count: int
    ) -> list[CollectedItem]:
        """
        ...
        try:
            segments = self._split_text_to_segments(
                text=normalized_text, max_tokens=self.config.max_tokens_per_segment
            )
            boundaries = (
                self._segment_boundaries if hasattr(self, "_segment_boundaries") else []
            )
        ...
        overlap_tokens = self._resolve_overlap_tokens(
            self.config.max_tokens_per_segment
        )
    ...
    def _build_segment_metadata(
        ...
        original_id: MetadataValue | None,
        segment_index: int,
        segment_count: int,
    ...
    async def _detect_language(self, text: str) -> str | None:
        ...
        try:
            ...
            logger.debug(
                f"[Preprocessor] 언어 감지 결과: {detected} (텍스트 길이: {len(text)})"
            )
            return detected
    ...
    def _is_spam(self, text: str) -> bool:
        ...
        for keyword in self.config.spam_keywords:
            if keyword.lower() in text_lower:
                logger.debug(
                    f"[Preprocessor] 스팸 키워드 발견: '{keyword}' in '{text[:50]}...'"
                )
                return True
    ...
    def _validate_segmentation_config(self) -> None:
        ...
        if self.config.segment_overlap_tokens >= self.config.max_tokens_per_segment:
            ...
            self.config.segment_overlap_tokens = max(
                self.config.max_tokens_per_segment - 1, 0
            )
    ...
    async def chunk_messages(
        self, records: list[dict[str, Any]], max_tokens: int = 50000
        ...
        for source, source_records in by_source.items():
            ...
            source_chunks = self._create_chunks_for_source(
                source_records=source_records, max_tokens=max_tokens, source_name=source
            )
        ...
        for i, chunk in enumerate(all_chunks):
            chunk_tokens = sum(self._estimate_record_tokens(record) for record in chunk)
            chunk_sources = {record.get("source", "unknown") for record in chunk}
    ...
    def _create_chunks_for_source(
        self, source_records: list[dict[str, Any]], max_tokens: int, source_name: str
    ) -> list[list[dict[str, Any]]]:
        """
    ...
    def _split_text_to_segments(
        ...
        return self._split_without_token_encoder(
            text=text, max_tokens=effective_max_tokens, overlap_tokens=overlap_tokens
        )
