...
class DataNormalizer:
    ...
    def normalize_articles(articles: List[Article]) -> List[CollectedItem]:
        ...
        for article in articles:
            try:
                ...
                logger.error(f"[DataNormalizer] Article 정규화 중 오류 발생: source={article.source}, error={e}")
                continue
    ...
    @staticmethod
    def normalize_telegram_messages(messages: List[TelegramMessage]) -> List[CollectedItem]:
        """
        ...
        logger.info(f"[DataNormalizer] TelegramMessage 정규화 완료: {len(normalized_items)}개")
        return normalized_items
    
    @staticmethod
    def normalize_mixed_data(
        articles: Optional[List[Article]] = None,
        telegram_messages: Optional[List[TelegramMessage]] = None
    ) -> List[CollectedItem]:
        """
        혼합된 데이터 소스를 정규화합니다.
        ...
        if telegram_messages:
            message_items = DataNormalizer.normalize_telegram_messages(telegram_messages)
            normalized_items.extend(message_items)
        
        # 시간순 정렬
        normalized_items.sort(key=lambda x: x.timestamp)
        
        logger.info(f"[DataNormalizer] 혼합 데이터 정규화 완료: {len(normalized_items)}개")
        return normalized_items
