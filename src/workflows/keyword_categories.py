...
MACRO_KEYWORDS = {
    # 금리 및 통화정책
    "금리", "기준금리", "금리인상", "금리인하", "금리동결", "금리정책", "금리 인하", "금리 인상",
    "연준", "fed", "federal reserve", "fomc", "제롬파월", "파월",
    "ecb", "유럽중앙은행", "boe", "영란은행", "일본은행", "boj", "한국은행", "bok",
    "중앙은행", "통화정책", "긴축", "완화",

    # 인플레이션 및 경제지표
    "인플레이션", "물가", "cpi", "pce", "ppi", "소비자물가지수", "inflation",
    "gdp", "경제성장률", "실업률", "고용", "비농업고용", "nfp",
    "pmi", "제조업지수", "소비자신뢰지수",
    "경제 성장", "경제 지표", "경기 전망",

    # 환율 및 통화
    "달러", "달러인덱스", "dxy", "환율", "원달러", "엔달러", "유로달러",
    "강달러", "약달러", "외환", "통화가치", "외환 시장", "fx",

    # 주식시장
    "나스닥", "nasdaq", "s&p500", "sp500", "다우존스", "dow",
    "주식시장", "증시", "코스피", "코스닥", "미국증시",
    "tech주", "기술주", "빅테크",

    # 채권 및 금리상품
    "국채", "채권", "수익률", "yield", "10년물", "2년물",
    "채권수익률", "장단기금리차", "역전",

    # 경기 및 경제 상황
    "경기침체", "recession", "경기둔화", "경기회복", "경기확장",
    "스태그플레이션", "디플레이션", "경기순환",
    "경제", "한국 경제", "미국 경제", "글로벌 경제","트럼프"

    # 정부 및 정책
    "재정정책", "양적완화", "qe", "qt", "양적긴축", "테이퍼링",
    "부채한도", "정부지출", "재정적자", "세금", "세율",
    ...
    # 은행 및 금융시스템
    "은행위기", "금융위기", "뱅크런", "예금보험", "금융시스템",
    "신용경색", "유동성위기", "svb", "실리콘밸리은행",
...
CRYPTO_NATIVE_KEYWORDS = {
    ...
    # 블록체인 기술
    "블록체인", "blockchain", "합의알고리즘", "pow", "pos",
    "샤딩", "sharding", "롤업", "rollup", "레이어1", "l1",
    "레이어2", "l2", "사이드체인", "sidechain",
    "제로지식증명", "zk", "zkp", "영지식증명",

    # DeFi (탈중앙화 금융)
    "디파이", "defi", "탈중앙화금융", "탈중앙화거래소", "dex",
    "유동성풀", "amm", "자동마켓메이커", "유동성마이닝",
    "yield farming", "이자농사", "스테이킹", "staking",
    "lending", "대출", "borrowing", "차입",
    "tvl", "총예치금", "프로토콜수익", "protocol revenue",
    "유니스왑", "uniswap", "커브", "curve", "aave", "컴파운드", "compound",

    # NFT 및 메타버스
    "nft", "non-fungible token", "대체불가토큰",
    "메타버스", "metaverse", "가상세계",
    "pfp", "generative art", "opensea", "블러", "blur",

    # 프로토콜 및 프로젝트
    "아비트럼", "arbitrum",
    "옵티미즘", "optimism", "베이스", "base", "zksync",

    # 온체인 데이터
    "온체인", "onchain", "해시레이트", "hashrate", "난이도", "difficulty",
    "네트워크활동", "트랜잭션", "transaction", "gas", "가스비",
    "밈풀", "mempool", "블록생성", "블록시간",
    "활성주소", "active address", "고래", "whale",

    # 토큰 경제학
    "토큰이코노미", "tokenomics", "토큰분배", "베스팅", "vesting",
    "에어드랍", "airdrop", "토큰발행", "mint", "burn", "소각",
    "인플레이션율", "emission", "리워드",

    # 거버넌스
    "dao", "거버넌스", "governance", "투표", "제안", "proposal",
    "커뮤니티", "decentralization", "탈중앙화",

    # 스마트컨트랙트 및 개발
    "스마트컨트랙트", "smart contract", "dapp", "디앱",
    "evm", "solidity", "rust", "move",
    "오픈소스", "github", "audit", "감사", "보안감사",

    # Web3 & 신기술 트렌드
    "web3", "웹3", "web3게임", "gamefi", "p2e",
    "socialfi", "depin", "탈중앙화id", "did",
    "ai", "인공지능", "ai agent", "ai crypto",
    "rwa", "real world assets", "실물자산토큰화",
    "meme", "밈코인", "meme coin",
    "디지털자산",
}
...
CRYPTO_MACRO_KEYWORDS = {
    # ETF 및 기관 투자
    "etf", "현물etf", "비트코인etf", "이더리움etf",
    "그레이스케일", "grayscale", "gbtc", "ethe",
    "블랙록", "blackrock", "피델리티", "fidelity",
    "ark", "캐시우드", "cathie wood",
    "기관투자", "기관자금", "기관 투자", "institutional", "월가", "wall street",

    # 상장 및 거래소
    "상장", "listing", "거래소", "exchange",
    "코인베이스", "coinbase", "바이낸스", "binance",
    "업비트", "빗썸", "kraken", "크라켄", "upbit",
    "해킹", "hacking", "보안", "security", "해킹/보안",
    "업비트 해킹", "바이낸스 해킹",

    # 규제 및 법률
    "sec", "증권거래위원회", "cftc", "finra",
    "규제", "regulation", "규제당국", "금융당국",
    "증권", "security", "상품", "commodity",
    "규제준수", "compliance", "라이센스", "면허",
    "소송", "lawsuit", "판결",
    "디지털 자산 규제", "암호화폐 규제",

    # CBDC 및 정부 디지털화폐
    "cbdc", "중앙은행디지털화폐", "디지털달러", "디지털위안",
    "디지털유로", "e-krw", "디지털원화",

    # 결제 및 채택
    "결제", "payment", "송금", "remittance", "크로스보더",
    "paypal", "페이팔", "visa", "비자", "mastercard", "마스터카드",
    "스트라이프", "stripe", "square", "스퀘어",
    "엘살바도르", "법정화폐", "legal tender",

    # 자산운용 및 투자상품
    "자산운용", "asset management", "헤지펀드", "hedge fund",
    "포트폴리오", "portfolio", "분산투자", "diversification",

    # 상관관계
    "상관관계", "correlation", "디커플링", "decoupling",
    "리스크온", "risk-on", "리스크오프", "risk-off",
    "안전자산", "safe haven", "위험자산", "risk asset",

    # 채택 및 메인스트림
    "채택", "adoption", "mainstream", "메인스트림",
    "대량채택", "mass adoption", "일반투자자", "retail",

    # 스테이블코인 (매크로와 크립토의 교차점)
    "스테이블코인", "stablecoin", "usdt", "usdc", "dai",
    "테더", "tether", "circle",
...
class KeywordCategorizer:
    ...
    def categorize_keyword_multilabel(
        ...
        # Macro 카테고리 매칭 확인
        macro_confidence = self._calculate_category_confidence(keyword_lower, self.macro_set)
    ...
    def _calculate_category_confidence(self, keyword_lower: str, category_set: set) -> float:
        """
    ...
    def categorize_keywords_multilabel(
        ...
        for kw_dict in keywords:
            ...
            multi_category = self.categorize_keyword_multilabel(term, category_thresholds)
