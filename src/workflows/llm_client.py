...
class GeminiClient:
    ...
    def __init__(
        ...
        self.model_type: Literal["flash", "pro"] = self._detect_model_type(
            normalized_model
        )
        ...
        if safety_settings is not None:
            ...
            logger.debug("[GeminiClient] 안전 설정 적용: 모든 카테고리 BLOCK_NONE")
        ...
        if hasattr(response, "usage_metadata") and response.usage_metadata:
            ...
            if isinstance(usage, dict):
                ...
                try:
                    usage_dict = (
                        dict(usage)
                        if hasattr(usage, "__iter__") and not isinstance(usage, str)
                        else {}
                    )
            ...
            if output_details:
                ...
                if not isinstance(output_details, dict):
                    try:
                        output_details = (
                            dict(output_details)
                            if hasattr(output_details, "__iter__")
                            and not isinstance(output_details, str)
                            else {}
                        )
                    except (TypeError, ValueError):
                        output_details = {
                            "reasoning": getattr(output_details, "reasoning", None)
                        }

                reasoning_tokens = (
                    output_details.get("reasoning", 0) if output_details else 0
                )
    ...
    def _extract_response_text(self, response: Any) -> str:
        ...
        if hasattr(response, "content"):
            ...
            if content is None:
                ...
                logger.error("[GeminiClient] 응답 객체 공개 속성: %s", public_attrs)
            ...
            if isinstance(content, list):
                ...
                logger.debug(
                    "[GeminiClient] content가 문자열입니다: 길이=%d", len(response_text)
                )
            ...
            if hasattr(response, "text"):
                ...
                logger.warning("[GeminiClient] 'text' 속성을 사용합니다 (비표준).")
                return response_text
                ...
                logger.error(
                    "[GeminiClient] 응답 객체 공개 속성: %s", public_attrs[:30]
                )
                raise ValueError(
    ...
    def _log_empty_response_debug(self, response: Any) -> None:
        ...
        if hasattr(response, "response_metadata"):
            ...
            logger.error(
                ...
                (
                    str(response_metadata)[:500]
                    if len(str(response_metadata)) > 500
        ...
        if hasattr(response, "additional_kwargs"):
            ...
            logger.error(
                ...
                (
                    str(additional_kwargs)[:500]
                    if len(str(additional_kwargs)) > 500
                    else str(additional_kwargs)
                ),
            )
        ...
        try:
            ...
            logger.error("[GeminiClient] 응답 객체 덤프 중 오류 발생: %s", str(e))
    ...
    async def generate_content_async(
        self, prompt: str, response_format: str | None = None, **kwargs
    ) -> str:
        """
