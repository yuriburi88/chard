...
async def collector_node(state: AnalysisState) -> AnalysisState:
    ...
    if not raw_records:
        ...
        return {
            **state,
            ...
            + ["CollectorNode: raw_records가 비어있습니다."],
        }
    ...
    if not filtered_records:
        logger.warning(
            "[CollectorNode] 키워드 추출 대상 레코드가 없습니다 (Economic Calendar만 존재)."
        )
        return {
            **state,
            "chunks": [],
            "chunk_count": 0,
            "errors": state.get("errors", [])
            + ["CollectorNode: 키워드 추출 대상 레코드가 없습니다."],
        }
    ...
    try:
        ...
        chunks = await preprocessor.chunk_messages(
            records=filtered_records, max_tokens=chunk_size
        )
    ...
    for chunk_id, chunk in enumerate(chunks):
        ...
        chunks_with_metadata.append(
            {"chunk_id": chunk_id, "records": chunk, "metadata": chunk_metadata}
        )
