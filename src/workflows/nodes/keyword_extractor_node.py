...
async def keyword_extractor_node(state: AnalysisState) -> AnalysisState:
    ...
    if not chunks:
        ...
        return {
            **state,
            "extracted_keywords": [],
            "errors": state.get("errors", []) + ["KeywordExtractorNode: chunks가 비어있습니다."]
        }
    ...
    # 5. 각 청크에 대해 키워드 추출 수행 (병렬 처리, 동시성 제한 가능)
    ...
    try:
        if parallel_concurrency == 0:
            ...
            tasks = [
                extract_keywords_from_chunk(chunk_with_ids, chunk_id=i, gemini_client=gemini_client)
            ...
            logger.info(f"[KeywordExtractorNode] 제한된 병렬 처리 모드로 실행 (최대 {parallel_concurrency}개 동시 실행)")
            semaphore = asyncio.Semaphore(parallel_concurrency)
            
            async def extract_with_semaphore(chunk_with_ids: List[Dict[str, Any]], chunk_id: int) -> Dict[str, Any]:
                """Semaphore를 사용하여 동시 실행 수를 제한하는 래퍼 함수"""
                async with semaphore:
                    return await extract_keywords_from_chunk(chunk_with_ids, chunk_id=chunk_id, gemini_client=gemini_client)
    ...
    # 6. 통계 로깅
    concurrency_info = "무제한" if parallel_concurrency == 0 else f"최대 {parallel_concurrency}개"
    ...
    logger.debug(
        f"[KeywordExtractorNode] ID 매핑 통계: {id_mapping_stats}"
    )
    
    # 7. state 업데이트 (ID 매핑 정보 포함)
    return {
        **state,
...
async def extract_keywords_from_chunk(
    ...
    """
    logger.debug(f"[KeywordExtractorNode] 청크 {chunk_id+1} 키워드 추출 시작: 레코드 수={len(chunk)}개")
    
    try:
        # 프롬프트 구성
        prompt = build_keyword_extraction_prompt(chunk)
