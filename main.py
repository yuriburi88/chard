...
from src.config_manager import AppConfig, ConfigManager, OutputConfig
from src.normalizer import DataNormalizer
from src.preprocessor import PreprocessingConfig, Preprocessor
from src.storage import ExecutionLogger, StorageManager
from src.utils.async_utils import run_async_main
...
def resolve_log_file_path(
    logs_dir: Path, execution_timestamp: datetime, output_config: OutputConfig
) -> Path:
    """로그 파일 경로를 계산합니다.
...
예시:
  ...
  python main.py --config path/to/config.yml
    ...
    parser.add_argument(
        ...
        help="설정 파일 경로 (기본값: config/config.yml)",
    )
...
def calculate_min_timestamp(app_config: AppConfig) -> datetime | None:
    """
    ...
    if period.start_time:
        try:
            ...
            logger.info(
                f"수집 기간: start_time 직접 지정 - {min_timestamp.isoformat()}"
            )
    ...
    if mode == "days_back" and period.days_back is not None:
        ...
        min_timestamp_local = datetime.combine(
            target_date, datetime.min.time(), tzinfo=display_tz
        )
    ...
    if period.recent_hours:
        min_timestamp = datetime.now(timezone.utc) - timedelta(
            hours=period.recent_hours
        )
        logger.info(
            f"수집 기간: recent_hours={period.recent_hours} - {min_timestamp.isoformat()} ~ 현재"
        )
        return min_timestamp
...
async def collect_rss_data(
    ...
    async with RSSCollector() as rss_collector:
        for rss_config in app_config.rss_sources:
            try:
                articles = await rss_collector.collect(
                    rss_config, min_published=min_timestamp
                )
                rss_articles.extend(articles)
                logger.info(
                    f"[RSS 수집] {rss_config.name}: {len(articles)}개 기사 수집"
                )
            except Exception as e:
                logger.error(
                    f"[RSS 수집] {rss_config.name} 수집 실패: {e}", exc_info=True
                )
    ...
    execution_logger.log_stage(
        ...
        success=True,
        details={
            "sources_count": len(app_config.rss_sources),
...
async def collect_telegram_data(
    ...
    for tg_config in app_config.telegram_sources:
        try:
            messages = await telegram_collector.collect(
                tg_config, min_timestamp=min_timestamp
            )
            telegram_messages.extend(messages)
            logger.info(
                f"[텔레그램 수집] {tg_config.name}: {len(messages)}개 메시지 수집"
            )
        except Exception as e:
            logger.error(
                f"[텔레그램 수집] {tg_config.name} 수집 실패: {e}", exc_info=True
            )
            execution_logger.log_error(
    ...
    execution_logger.log_stage(
        ...
        success=True,
        details={
            "sources_count": len(app_config.telegram_sources),
            "messages_count": len(telegram_messages),
        },
    )
...
async def collect_economic_calendar_data(
    app_config: AppConfig, execution_logger: ExecutionLogger
) -> list[CollectedItem]:
    """
...
async def preprocess_data(
    ...
    # 병합된 아이템 전처리
    if combined_items:
        preprocessed_items = await preprocessor.preprocess_collected_items(
            combined_items
        )
        logger.info(
...
async def save_raw_data_to_file(
    ...
    raw_file_path = await storage_manager.save_raw_data(
        raw_data, output_dir, filename_prefix="collected", timestamp=execution_timestamp
    )
...
async def _save_analysis_results(
    ...
    markdown_report = build_markdown_report(
        final_state, timezone_offset=app_config.output.display_timezone
    )
    ...
    logger.info(
        f"통합된 키워드 수: {len(final_state.get('aggregated_keywords', []))}개"
    )
    ...
    if aggregated_keywords:
        ...
        for idx, keyword in enumerate(aggregated_keywords[:5], start=1):
            ...
            logger.info(f"  {idx}. {term} (점수: {score:.1f}, 출현: {occurrence}회)")
...
def print_execution_summary(
    execution_logger: ExecutionLogger, output_dir: Path
) -> None:
    """
    실행 완료 요약을 출력합니다.
    ...
    logger.info(
        f"텔레그램 메시지 수집: {summary['statistics'].get('telegram_messages', 0)}개"
    )
...
async def main_async(app_config: AppConfig, log_level: str = "INFO") -> None:
    ...
    try:
        ...
        # 1. 데이터 수집
        rss_articles = await collect_rss_data(
            app_config, min_timestamp, execution_logger
        )
        telegram_messages = await collect_telegram_data(
            app_config, min_timestamp, execution_logger
        )
        economic_events = await collect_economic_calendar_data(
            app_config, execution_logger
        )

        # 모든 소스가 없는 경우 에러 발생
        ec_enabled = app_config.economic_calendar.enabled
        if (
            not app_config.rss_sources
            and not app_config.telegram_sources
            and not ec_enabled
        ):
        ...
        normalized_items = await normalize_data(
            preprocessed_articles, preprocessed_items, execution_logger
        )
        ...
        if raw_file_path and normalized_items:
            ...
            try:
                ...
                from src.workflows.langgraph_pipeline import (
                    run_keyword_analysis_workflow,
                )
                from src.workflows.state import AnalysisState
